...
- `Crossfade` — when `contentKey` changes, the previous view exits in `direction` (`up` | `down` | `left` | `right`, default `left`) and the next view enters from the opposite side. `contentKey` is required. Do not put `key` on `Crossfade` itself or the exit is skipped. Defaults: `mode` `sync`, enter spring (`visualDuration` 0.3, `bounce` 0.2), exit `motionDurationMs` / `motionEasing`, offset spacing 6.
