...
- `Crossfade` — when `contentKey` changes, fades the previous view out in `direction` (`up` | `down` | `left` | `right`), then fades the new view in from the opposite side. `contentKey` is required. Do not put `key` on `Crossfade` itself or the exit is skipped. Optional `enterTransition` / `exitTransition` / `mode` / `offset` default to Maui motion tokens (`motionDurationMs`, `motionEasing`, spacing 6).
