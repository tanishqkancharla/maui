...
- Simple apps default to a `proseMaxWidth` column centered in their container with `width: "100%"`, `maxWidth: proseMaxWidth`, and `marginInline: "auto"`. `sizing.contentWidth` uses that same value. Use it for single-column tools, settings, forms, and reading layouts. Multi-pane or full-bleed apps such as inboxes, calendars, and IDEs are exceptions.
